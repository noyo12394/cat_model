...
type Tab = "summary" | "vulnerability" | "damage" | "probabilistic" | "audit";
    ...
    <div className="analytics-chart-wrap">
      <svg className="analytics-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Flood depth-damage vulnerability curves">
        ...
        {curves.map((curve, index) => {
          ...
          return <polyline key={curve.function_id} points={points} fill="none" stroke={CURVE_COLORS[index % CURVE_COLORS.length]} strokeWidth={usedIds.has(curve.function_id) ? 4 : 2} opacity={usedIds.has(curve.function_id) ? 1 : 0.48} />;
        })}
...
function EPChart({ probabilistic }: { probabilistic: ProbabilisticResult }) {
  ...
  return <div className="analytics-chart-wrap">
    <svg className="analytics-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Occurrence and aggregate exceedance probability curves">
      ...
      <polyline points={line(probabilistic.aep_curve)} fill="none" stroke="#1a73e8" strokeWidth="4" />
      <polyline points={line(probabilistic.oep_curve)} fill="none" stroke="#7e57c2" strokeWidth="3" />
      <text x={width / 2} y={height - 2} textAnchor="middle" className="axis-title">Return period (log scale)</text>
    </svg>
    <div className="chart-legend"><span><i style={{ background: "#1a73e8" }} />AEP · annual aggregate</span><span><i style={{ background: "#7e57c2" }} />OEP · largest event</span></div>
  </div>;
}
...
export function ModelAnalytics({ run, curves, probabilistic }: { run: CatModelRunResult; curves: VulnerabilityFunction[]; probabilistic: ProbabilisticResult | null }) {
  ...
  return <div className="model-analytics">
    <nav className="analytics-tabs" aria-label="Model result views">
      {(["summary", "vulnerability", "damage", "probabilistic", "audit"] as Tab[]).map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item === "probabilistic" ? "EP & tail" : item}</button>)}
    </nav>

    {tab === "summary" && <section className="analytics-section">
      <div className="result-hero"><span>Modelled ground-up loss range</span><strong>{money(run.ground_up_distribution.range_low_usd)}–{money(run.ground_up_distribution.range_high_usd)}</strong><small>Median {money(run.ground_up_distribution.p50_usd)} · {run.ground_up_distribution.samples.toLocaleString()} Monte Carlo samples</small></div>
    ...
    {tab === "vulnerability" && <section className="analytics-section">
    ...
    {tab === "damage" && <section className="analytics-section">
    ...
    {tab === "probabilistic" && <section className="analytics-section">
    ...
    {tab === "audit" && <section className="analytics-section">
